...
# Red interna Docker: 172.20.0.x  →  zona inversa db.172.20.0
ZONE_INVERSA="/etc/bind/db.172.20"
# Contraseña root de MariaDB (viene de variable de entorno del contenedor o valor por defecto)
MYSQL_ROOT_PASS="${MYSQL_ROOT_PASSWORD:-rootpass123}"
# Host de MariaDB: el contenedor marisma-db en la red Docker
MYSQL_HOST="${MYSQL_HOST:-172.20.0.20}"
SOLO_SSH=false
...
useradd -m -s /bin/bash -d "${DOCUMENT_ROOT}" "$USER"
...
apache2ctl configtest > /dev/null 2>&1 && apache2ctl graceful
...
service bind9 reload > /dev/null 2>&1 || rndc reload > /dev/null 2>&1
log_ok "BIND9 recargado"

# ─── 5. BASE DE DATOS MYSQL ───────────────────────────────────────────────────
log_info "5/7 Creando base de datos MySQL..."

mariadb -h "${MYSQL_HOST}" -u root -p"${MYSQL_ROOT_PASS}" <<SQL 2>/dev/null
...
if [ "$SOLO_SSH" = true ]; then
    ...
    service proftpd reload > /dev/null 2>&1
